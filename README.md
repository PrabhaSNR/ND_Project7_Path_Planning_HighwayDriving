...
with every update cycle we need to know the state of our car (lane, velocity and acceleration) and the state of the adjacent vehicles (and their future state) in order to plan our next state, which basically will consist in defining the change of velocity and whether or not we need to change lanes. Finally, based on determining future state, we need to generate the corresponding trajectory.

So, the solution has two steps/areas to deal with: (1) behavioral planning, and (2) trajectory generation. 
...
we predict future state of other vehicles so that we can use this information to plan our behaviour in order to run as fast as possible and to prevent any collision with other cars. 
